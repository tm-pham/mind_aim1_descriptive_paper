...
# 1. Prepare input and output data (lines 26-43). 
# 2. Run GEE analysis for each organism (lines 44-96). 
# 3. Compute multiplicative Average Annual Percentage Change (lines 98-141). 
# 4. Save output data (lines 142-150). 
...
output_table <- cbind(df_gee_results, text = apply(df_gee_results, 1, 
                                                   function(x) paste0(x[4], "% (95%CI: [", x[5], ", ", x[6], "])"))) %>% 
  mutate(org = factor(org, levels=bugs_sorted)) %>% 
  arrange(org)
...
for(i in 1:nrow(output_table)){
  if(output_table[i, "asymp.LCL"]<=0 & 0<=output_table[i, "asymp.UCL"]){
    ...
    output_table[i, "text.star"] <- paste0(output_table[i, "time.trend"], "*")
  }
}
...
# 4. Save Data
# Save as Excel sheet
write.xlsx(output_table, sep="\t", file = "data/mind_inc_gee_results_by_org_onset_geeasy_year_Dec23.xlsx", 
